...
class XetraTargetConfig(NamedTuple):
    ...
    trg_coll_isin: column name for isin in target
    ...
    trg_col_ch_prev_clos: column name for change from previous close in target
    trg_key:basic key for target file
    trg_key_date_format: date format for target file key
    trg_foramt: file format for target file
...
class XetraETL():
    """
    Read the Xetra data from S3, transform it and write it back to S3
    """
    ...
    def __init__(self, s3_bucket_src: S3BucketConnector
        ...
        """
        Constructor for XetraETL class

        :param s3_bucket_src: connection to source S3 bucket
        :param s3_bucket_trg: connection to target S3 bucket
        :param meta_key: key for meta file
        :param src_args: source configuration data
        :param trg_args: target configuration data
        """
