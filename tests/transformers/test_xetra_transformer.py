...
class TestXetraETLMethods(unittest.TestCase):
    ...
    def test_load(self):
        ...
        # Check written target file
        trg_files = self.s3_bucket_trg.list_files_in_prefix(self.target_config.trg_key)
        print("Target files found:", trg_files)
        self.assertTrue(len(trg_files) > 0, "No files were written to target S3.")
        ...
        # Check meta file
        meta_files = self.s3_bucket_trg.list_files_in_prefix(self.meta_key)
        print("Meta files found:", meta_files)
        self.assertTrue(len(meta_files) > 0, "No meta files found in target S3.")
